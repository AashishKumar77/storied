...
const SearchForm = ({
  ...
  const formValidate = (_values) => {
    ...
    if (
      ...
      _values.ln.t === "" &&
      _values.sr === "" &&
      (_values.Residence.name === "" || _values.Residence.name === undefined)
    ) {
  ...
  const { race, dropdownLoading } = useSelector((state) => { return state.usCensus1820; });
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    handleSubmitUSCensus1820(valuesData, { setSubmitting });
  };
            ...
            <Form className="w-full">
              <div className="flex flex-wrap -mx-2 md:mb-2.5">
                <div className={`w-full ${width} px-2 mb-2.5`}>
                  {inputField(tr(t, "search.ww1.form.fmname"), 'fm', values, setFieldValue, handleChange, defaultUSFederal1820Search, t)}
                </div>
                <div className={`w-full ${width} px-2 mb-2.5`}>
                  {inputField(tr(t, "search.ww1.form.lname"), 'ln', values, setFieldValue, handleChange, defaultUSFederal1820Search, t)}
                </div>
              </div>
              <div className="flex flex-wrap -mx-2 md:mb-2.5">
                <div className={`w-full ${width} px-2 mb-2.5`}>
